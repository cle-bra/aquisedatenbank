<?php
// public/companies.php – Firmenliste mit Sortierung, Filtern, Bearbeiten-Link und Kampagnenübersicht
...
// ---- Optionen ----
$allowedSort = ['name','city','zip','website','email_general','phone_general'];
...
if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['action']??'')==='bulk_add_companies_to_campaign'){
  if (!hash_equals($_SESSION['csrf'] ?? '', $_POST['csrf'] ?? '')) { flash('error','Ungültiges Formular (CSRF).'); header('Location: /companies.php'); exit; }
  $targetCampaign = (int)($_POST['target_campaign_id'] ?? 0);
  $ids = array_filter(array_map('intval', $_POST['selected'] ?? []));
  if ($targetCampaign<=0 || empty($ids)) { flash('error','Bitte Kampagne und mindestens eine Firma wählen.'); header('Location: /companies.php'); exit; }
  $ok=0; $fail=0;
  try {
  ...
  header('Location: /companies.php?'.http_build_query($_GET)); exit;
}
...
// ---- Query bauen ----
$where=[]; $params=[];
if ($q!==''){ $where[] = "(co.name LIKE :q OR co.city LIKE :q OR co.zip LIKE :q OR co.website LIKE :q OR co.email_general LIKE :q)"; $params[':q'] = "%$q%"; }
...
// Fetch rows inkl. Kampagnen
$rows=[]; try {
  $stmt=$pdo->prepare('SELECT co.id, co.name, co.street, co.zip, co.city, co.website, co.email_general, co.phone_general, 
    (SELECT GROUP_CONCAT(ca.name SEPARATOR ", ") FROM campaign_companies cc JOIN campaigns ca ON ca.id=cc.campaign_id WHERE cc.company_id=co.id) AS campaigns '.$baseSQL.' ORDER BY '.$orderBy.' LIMIT :lim OFFSET :off');
...
function sort_link($key,$label){
  ...
  $qs = $_GET; $qs['sort']=$key; $qs['dir']=$nextDir; $url='/companies.php?'.http_build_query($qs);
    ...
    <div class="card-body">
      <form class="row g-2 align-items-end" method="get">
        <div class="col-12 col-md-4">
          ...
          <input type="text" name="q" class="form-control" value="<?= e($q) ?>" placeholder="Firma, Stadt, PLZ, Website, E‑Mail">
        </div>
        ...
        <div class="col-12 mt-2">
          <div class="row g-2 align-items-end">
            ...
            <div class="col-12 col-md-3 col-lg-2">
              <a class="btn btn-outline-secondary w-100" href="/companies.php">Zurücksetzen</a>
            </div>
          </div>
        </div>
      </form>
        ...
        <table class="table table-striped table-hover align-middle">
          <thead>
            <tr>
              ...
              <th><?= sort_link('website','Webseite') ?></th>
              <th><?= sort_link('email_general','E‑Mail (allg.)') ?></th>
          ...
          <tbody>
            <?php if(empty($rows)): ?>
              <tr><td colspan="10" class="text-muted">Keine Firmen gefunden.</td></tr>
            <?php else: foreach($rows as $r): ?>
              <tr>
                ...
                <td><?= e($r['zip'] ?: '—') ?></td>
                <td><?= e($r['city'] ?: '—') ?></td>
                <td><a href="<?= e($r['website']) ?>" target="_blank" rel="noopener noreferrer"><?= e($r['website'] ?: '—') ?></a></td>
                <td><a href="mailto:<?= e($r['email_general']) ?>"><?= e($r['email_general'] ?: '—') ?></a></td>
                <td><?= e($r['phone_general'] ?: '—') ?></td>
                <td><?= e($r['campaigns'] ?: '—') ?></td>
                <td class="text-end"><a href="/public/company_edit.php?id=<?= (int)$r['id'] ?>" class="btn btn-sm btn-outline-secondary">Bearbeiten</a></td>
              </tr>
