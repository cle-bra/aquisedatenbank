          ...
          <tbody>
            ...
            <?php else: foreach($rows as $r): ?>
              <?php
                ...
                $hist=[]; $hSt=$pdo->prepare('SELECT c.id, c.status, c.sentiment, c.callback_at, c.created_at, u.full_name AS agent FROM calls c LEFT JOIN users u ON u.id=c.agent_id WHERE c.campaign_id=:ca AND c.company_id=:co ORDER BY c.created_at DESC LIMIT 3');
              ...
              <tr class="<?= $r['removed']? 'table-secondary' : '' ?>">
                ...
                <td class="text-end">
                  <div class="btn-group btn-group-sm">
                    <?php if(!$r['removed']): ?>
                      <a class="btn btn-outline-primary" href="/call_add.php?company_id=<?= (int)$r['id'] ?>&campaign_id=<?= (int)$campId ?>">📞</a>
